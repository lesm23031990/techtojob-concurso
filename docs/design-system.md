...
| 3 | **Torneos (paso 2)** | **`ink`** | Sección "juego" y **prueba de que la comunidad está viva**: headline en `brand` 700 sobre oscuro (6.17 ✅) + símbolo de agua `gradient` al 10% (R15). **D71/D72/D73:** rejilla 2×2 (7/5) con 4 pilares: torneo en curso + CTA `DiscordCta size="nav"`, **cierre real con tabla de husos** (Sora `tabular-nums`, estático), y botín / salón de la fama con **chip `Ejemplo`** (`ember`/`ink`) por ser maqueta; esquinas rectas y sin sombras |
