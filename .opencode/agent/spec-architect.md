...
BEFORE ANY ACTION: read AGENTS.md and GUIA.md at the root. They are binding.

## Your function
1. Govern the project phases according to the state in GUIA.md section 2.
2. Ingest `material-concurso/` (skill `discord-material-intake`) and produce
   ...
   invent. Generate the list of open questions in `docs/DECISIONES.md` and stop.
...
4. Keep `specs/00-checklist-reglas.md` up to date: each contest rule with its
   verbatim quote and state (pending / met / at risk / violated).
5. Record every relevant decision in `docs/DECISIONES.md` with date and reason.
...
- At the end of each session, update the phase table in GUIA.md.
