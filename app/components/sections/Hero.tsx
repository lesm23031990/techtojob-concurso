  ...
  return (
    ...
    >
      ...
      <div className="page-container hero-soft relative flex flex-1 flex-col justify-center pt-36 pb-16 lg:grid lg:grid-cols-12 lg:content-center lg:pt-40 lg:pb-20">
        ...
        <h1
          id="hero-heading"
          className="mt-0 text-display-sm font-bold tracking-tight text-pretty sm:text-display md:text-display-md lg:col-start-2 lg:col-span-8 lg:mt-0 lg:text-display-lg"
        >
      ...
      {/* Ticker (D38/D96): the shared band lives in `components/Ticker.tsx`
          since D122, so the hero and the closing render exactly the same
          markup and cannot diverge. It is the hero's last row (and the closing's
          too: the page opens and closes with the same band). */}
      <Ticker />
    </section>
  );
}
