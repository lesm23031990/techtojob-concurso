...
export default function HowItWorks() {
  return (
    <Section id="como-funciona" headingId="como-funciona-heading" tone="paper">
      ...
      >
        {messages.comoFunciona.h2}
      </h2>
      <p className="mt-4 max-w-prose text-lead text-slate">
        {messages.comoFunciona.intro}
      </p>

      <ol className="mt-12 grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-4">
        {messages.comoFunciona.pasos.map((step, index) => (
          <li key={step.titulo} className="relative flex flex-col gap-4">
            {/* horizontal connector between circles, lg only (§6.8) */}
            {index < messages.comoFunciona.pasos.length - 1 && (
              <span
                aria-hidden="true"
            ...
            <h3 className="text-h3 font-semibold lg:text-h3-lg">{step.titulo}</h3>
            <p className="max-w-prose text-body">{step.texto}</p>
          </li>
        ))}
      </ol>

      <p className="mt-12 border-t border-line pt-6 text-lead font-semibold">
        {messages.comoFunciona.cierre}
      </p>
    </Section>
  );
}
